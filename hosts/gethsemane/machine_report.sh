...
# █░▀░█ █▀█ █▄▄ █▀█ █ █░▀█ ██▄   █▀▄ ██▄ █▀▀ █▄█ █▀▄ ░█░
#
# TR-100-style machine report, printed on every interactive shell
#
...
# gethsemane-specific: syncthing overall completion via local REST API
get_syncthing_status() {
    local config_path="/home/vessel/.config/syncthing/config.xml"
    if [ ! -r "$config_path" ]; then
        printf 'config unreadable'
        return
    fi
    local key
    key=$(grep -oP '(?<=<apikey>)[^<]+' "$config_path")
    local completion
    completion=$(curl -s --max-time 2 -H "X-API-Key: $key" http://127.0.0.1:8384/rest/db/completion | jq -r '.completion // empty')
    if [ -z "$completion" ]; then
        printf 'unreachable'
    elif awk -v c="$completion" 'BEGIN{exit !(c>=100)}'; then
        printf 'up to date'
    else
...
get_restic_status() {
    local unit="restic-backups-$1.service"
    local result start
    result=$(systemctl show "$unit" -p Result --value 2>/dev/null)
    start=$(systemctl show "$unit" -p ExecMainStartTimestamp --value 2>/dev/null)
    if [ -z "$result" ]; then
        printf 'never run'
        return
    fi
    local when
    when=$(date -d "$start" +%m/%d 2>/dev/null || echo "?")
    printf '%s (%s)' "$result" "$when"
}
