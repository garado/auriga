...
export const SmartStack = (props: { 
  ...
  const defaultTransition = props.vertical ? 
    Gtk.StackTransitionType.SLIDE_DOWN : Gtk.StackTransitionType.SLIDE_UP
  ...
  const Stack = Widget.Stack({
    ...
    setup: self => {
      props.children.map(c => { self.add_named(c.ui(), c.name) })
    }
  })

  if (props.bindNamedSwitchTo) {
    props.bindNamedSwitchTo.subscribe(newName => {
      const currentName = Stack.get_visible_child_name()
      if (currentName === newName) return
      
      const isNext = childIndices.indexOf(currentName) < childIndices.indexOf(newName)

      if (props.vertical) {
        Stack.set_transition_type(isNext ? Gtk.StackTransitionType.SLIDE_UP : Gtk.StackTransitionType.SLIDE_DOWN)
      } else {
        Stack.set_transition_type(isNext ? Gtk.StackTransitionType.SLIDE_LEFT : Gtk.StackTransitionType.SLIDE_RIGHT)
      }
  ...
  if (props.bindNumberedSwitchTo) {
    props.bindNumberedSwitchTo.subscribe(newIndex => {
